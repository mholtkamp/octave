...
struct DockContext
{
    ...
    struct Dock
    {
        ...
        static constexpr float kSplitterThickness = 5.0f;
        ...
        ImVec2 getMinSize() const
        {
            ...
            return isHorizontal() ? ImVec2(s0.x + s1.x + kSplitterThickness, ImMax(s0.y, s1.y))
                                  : ImVec2(ImMax(s0.x, s1.x), s0.y + s1.y + kSplitterThickness);
        }
        ...
        void setChildrenPosSize(const ImVec2& _pos, const ImVec2& _size)
        {
            ...
            if (isHorizontal())
            {
                float avail = _size.x - kSplitterThickness;
                s.y = _size.y;
                s.x = (float)int(
                    avail * children[0]->size.x / (children[0]->size.x + children[1]->size.x));
                if (s.x < children[0]->getMinSize().x)
                    s.x = children[0]->getMinSize().x;
                else if (avail - s.x < children[1]->getMinSize().x)
                    s.x = avail - children[1]->getMinSize().x;
                children[0]->setPosSize(_pos, s);

                s.x = avail - children[0]->size.x;
                ImVec2 p = _pos;
                p.x += children[0]->size.x + kSplitterThickness;
                children[1]->setPosSize(p, s);
            }
            else
            {
                float avail = _size.y - kSplitterThickness;
                s.x = _size.x;
                s.y = (float)int(
                    avail * children[0]->size.y / (children[0]->size.y + children[1]->size.y));
                if (s.y < children[0]->getMinSize().y)
                    s.y = children[0]->getMinSize().y;
                else if (avail - s.y < children[1]->getMinSize().y)
                    s.y = avail - children[1]->getMinSize().y;
                children[0]->setPosSize(_pos, s);

                s.y = avail - children[0]->size.y;
                ImVec2 p = _pos;
                p.y += children[0]->size.y + kSplitterThickness;
                children[1]->setPosSize(p, s);
            }
        }
    ...
    void splits()
    {
        ...
        for (int i = 0; i < m_docks.size(); ++i)
        {
            ...
            {
                float sp = Dock::kSplitterThickness;
                cursor = ImGuiMouseCursor_ResizeEW;
                SetCursorScreenPos(ImVec2(dock.pos.x + size0.x, dock.pos.y));
                InvisibleButton("split", ImVec2(sp, dock.size.y));
                ...
                size1 -= dsize;
                pos0 = dock.pos;
                pos1.x = pos0.x + size0.x + sp;
                pos1.y = dock.pos.y;
                size0.y = size1.y = dock.size.y;
                size1.x = ImMax(min_size1.x, dock.size.x - size0.x - sp);
                size0.x = ImMax(min_size0.x, dock.size.x - size1.x - sp);
            }
            else
            {
                float sp = Dock::kSplitterThickness;
                ...
                InvisibleButton("split", ImVec2(dock.size.x, sp));
                ...
                pos0 = dock.pos;
                pos1.x = dock.pos.x;
                pos1.y = pos0.y + size0.y + sp;
                size0.x = size1.x = dock.size.x;
                size1.y = ImMax(min_size1.y, dock.size.y - size0.y - sp);
                size0.y = ImMax(min_size0.y, dock.size.y - size1.y - sp);
            }
    ...
    bool tabbar(Dock& dock, bool close_button)
    {
        ...
        if (BeginChild(tmp, size, true))
        {
            ...
            while (dock_tab)
            {
                ...
                {
                    sz.x += 16 + GetStyle().ItemSpacing.x;
                    SameLine();
    ...
    static void setDockPosSize(Dock& dest, Dock& dock, ImGuiDockSlot dock_slot, Dock& container, float ratio = 0.5f)
    {
        ...
        {
            case ImGuiDockSlot_Bottom:
                avail = container.size.y - sp;
                dock.size.y = avail * ratio;
                dest.size.y = avail - dock.size.y;
                dock.pos.y += dest.size.y + sp;
                break;
            case ImGuiDockSlot_Right:
                avail = container.size.x - sp;
                dock.size.x = avail * ratio;
                dest.size.x = avail - dock.size.x;
                dock.pos.x += dest.size.x + sp;
                break;
            case ImGuiDockSlot_Left:
                avail = container.size.x - sp;
                dock.size.x = avail * ratio;
                dest.size.x = avail - dock.size.x;
                dest.pos.x += dock.size.x + sp;
                break;
            case ImGuiDockSlot_Top:
                avail = container.size.y - sp;
                dock.size.y = avail * ratio;
                dest.size.y = avail - dock.size.y;
                dest.pos.y += dock.size.y + sp;
                break;
    ...
    void doDock(Dock& dock, Dock* dest, ImGuiDockSlot dock_slot, float ratio = 0.5f)
    {
...
static void* readOpen(ImGuiContext* ctx, ImGuiSettingsHandler* handler, const char* name)
{
    ...
    else if (tag.substr(0, 5) == "Size:")
    {
        ...
        std::string sizeStr = tag.substr(5);
        int dockSize = atoi(sizeStr.c_str());
...
void ImGui::SetNextDockPos(const char* panel, ImGuiDockSlot slot, float ratio)
{
    (void)ratio; // TODO: implement ratio-based splitting
    SetNextDock(panel, slot);
}

void ImGui::ResetNextDockParent(const char* panel)
{
...
void ImGui::UndockAll(const char* panel)
{
    if (!panel) return;
    ...
    // Collect leaf (non-container) docks
    ImVector<DockContext::Dock*> leaves;
    for (int i = 0; i < ctx.m_docks.size(); ++i)
    {
        if (!ctx.m_docks[i]->isContainer())
            leaves.push_back(ctx.m_docks[i]);
    }

    // Undock all leaves from back to front (preserves first as root)
...
void ImGui::DockTo(const char* panel, const char* dockLabel, const char* destLabel, ImGuiDockSlot slot, float ratio)
{
    if (!panel) return;
    auto it = g_docklist.find(panel);
    if (it == g_docklist.end()) return;
    ...
    // Build full labels (same format BeginDock uses)
    char dock_full[256], dest_full[256];
    snprintf(dock_full, sizeof(dock_full), "%s##%s", dockLabel, panel);
    snprintf(dest_full, sizeof(dest_full), "%s##%s", destLabel, panel);

    ImU32 dock_id = ImHashStr(dock_full, 0);
    ImU32 dest_id = ImHashStr(dest_full, 0);
    ...
    for (int i = 0; i < ctx.m_docks.size(); ++i)
    {
        if (ctx.m_docks[i]->id == dock_id) dock = ctx.m_docks[i];
        if (ctx.m_docks[i]->id == dest_id) dest = ctx.m_docks[i];
    }

    if (!dock || !dest || dock == dest) return;
    if (dock->parent) ctx.doUndock(*dock);
    ctx.doDock(*dock, dest, slot, ratio);
}

void ImGui::DockToRoot(const char* panel, const char* dockLabel, ImGuiDockSlot slot, float ratio)
{
    if (!panel) return;
    auto it = g_docklist.find(panel);
    if (it == g_docklist.end()) return;
    ...
    DockContext::Dock* root = ctx.getRootDock();
    if (!root || dock == root) return;

    if (dock->parent) ctx.doUndock(*dock);
    ctx.doDock(*dock, root, slot, ratio);
}

bool ImGui::HasDockLayout(const char* panel)
{
