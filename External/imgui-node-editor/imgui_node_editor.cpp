...
ed::EditorAction::AcceptResult ed::NavigateAction::Accept(const Control& control)
{
    ...
    int navBtn = Editor->GetConfig().NavigateButtonIndex;
    bool navDragging = (navBtn >= 0) && ImGui::IsMouseDragging(navBtn, 0.0f);
    bool ctrlLeftDragging = ImGui::GetIO().KeyCtrl && ImGui::IsMouseDragging(0, 0.0f);
    int activeBtn = ctrlLeftDragging ? 0 : navBtn;
    if (Editor->CanAcceptUserInput() /*&& !ImGui::IsAnyItemActive()*/ && (navDragging || ctrlLeftDragging))
    {
        m_IsActive    = true;
        m_ScrollStart = m_Scroll;
        m_ScrollDelta = ImGui::GetMouseDragDelta(activeBtn);
...
bool ed::NavigateAction::Process(const Control& control)
{
    ...
    bool ctrlLeftDragging = ImGui::GetIO().KeyCtrl && ImGui::IsMouseDragging(0, 0.0f);
    int activeBtn = ctrlLeftDragging ? 0 : navBtn;
    if (navDragging || ctrlLeftDragging)
    {
        m_ScrollDelta = ImGui::GetMouseDragDelta(activeBtn);
        m_Scroll      = m_ScrollStart - m_ScrollDelta * m_Zoom;
